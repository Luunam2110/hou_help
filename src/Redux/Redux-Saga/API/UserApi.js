...
function* getUserDetail(action) {
    ...
    yield fetch(USER_URL.USER_INFOR, {
        ...
        .then(([status, data]) => {
            ...
            if (res.status === 200) {
                res.name = data['name'];
                res.type_account = data['type_account'];
                res.avatarId = data['avatar'];
                res.access_token_stringee = data['access_token_stringee'];
                res.access_token_mbaas = data['access_token_mbaas'];
                res.unit = data['unit'];
            } else {
                console.log("ERROR_USER_API: " + data['error_description']);
                // res.error = data['error_description'];
...
function* changePassworddApi(action) {
    ...
    let url=`${USER_URL.CHANGE_PASSWORD}?oldpassword=${details.oldpassword}&newpassword=${details.newpassword}`;
...
export const userApi = {
    loginUserApi, getUserDetail, logoutUserApi,changePassworddApi
}
